                ...
                <div className="grid gap-6 py-4">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <p className="mt-1 text-gray-800">{roomType.description}</p>
                        </div>
                         <div>
                            <Label className="text-sm font-semibold text-gray-500">Amenities</Label>
                            <p className="mt-1 text-gray-800">{roomType.amenities || "No amenities listed."}</p>
                        </div>
                        <div className="grid grid-cols-3 gap-4 pt-4 border-t">
                            <div>
                                <Label className="text-sm font-semibold text-gray-500">Price</Label>
                                <p className="mt-1 text-lg font-bold text-gray-800">${parseFloat(roomType.price).toFixed(2)} <span className="text-sm font-normal">/ night</span></p>
                            </div>
                             <div>
                                <Label className="text-sm font-semibold text-gray-500 flex items-center"><Users className="w-4 h-4 mr-1"/> Adults</Label>
                                <p className="mt-1 text-lg font-bold text-gray-800">{roomType.capacity_adults}</p>
                            </div>
                             <div>
                                <Label className="text-sm font-semibold text-gray-500 flex items-center"><Baby className="w-4 h-4 mr-1"/> Children</Label>
                                <p className="mt-1 text-lg font-bold text-gray-800">{roomType.capacity_children}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <form onSubmit={submit} className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input id="price" type="number" value={data.price} onChange={e => setData('price', e.target.value)} className="col-span-3" />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="capacity_adults" className="text-right">Adults</Label>
                        <Input id="capacity_adults" type="number" value={data.capacity_adults} onChange={e => setData('capacity_adults', e.target.value)} className="col-span-3" />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="capacity_children" className="text-right">Children</Label>
                        <Input id="capacity_children" type="number" value={data.capacity_children} onChange={e => setData('capacity_children', e.target.value)} className="col-span-3" />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="amenities" className="text-right">Amenities</Label>
                        <Input id="amenities" value={data.amenities} onChange={e => setData('amenities', e.target.value)} className="col-span-3" />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <DialogFooter>
                        <Button type="submit" disabled={processing}>Add Room Type</Button>
                    </DialogFooter>
                </form>
            </DialogContent>
        </Dialog>
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Make changes to the room type here. Click save when you're done.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
        ...
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent>
                ...
                <AlertDialogFooter>
                    ...
                    <AlertDialogAction onClick={() => onConfirm(password)}>Continue</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
...
const RoomTypes = ({ roomTypes, flash, errors }) => {
    ...
    const [isEditRoomTypeDialogOpen, setEditRoomTypeDialogOpen] = useState(false);
    ...
    const confirmDelete = (password) => {
        router.delete(route("admin.room_types.destroy", { roomType: selectedRoomType.id }), {
            data: { password },
                    ...
                    {roomTypes.map(type => (
                        <div key={type.id} className="bg-white rounded-2xl shadow-md border border-gray-200/80 overflow-hidden flex flex-col">
                            ...
                            <div className="p-4 flex-grow flex flex-col">
                                <div className="cursor-pointer flex-grow" onClick={() => handleViewDetails(type)}>
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-200 flex justify-end items-center gap-2">
                                    ...
                                    <Button variant="destructive" size="icon" onClick={() => handleDelete(type)}>
