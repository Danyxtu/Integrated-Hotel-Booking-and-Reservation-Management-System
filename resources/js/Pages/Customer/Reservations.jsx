...
const Reservations = () => {
    ...
    const renderReservationCard = (reservation) => {
        ...
        const taxesAndFees = (basePricePerNight * totalNights * 0.15).toFixed(2); // 15% tax/fee
        const totalAmount = (basePricePerNight * totalNights + parseFloat(taxesAndFees)).toFixed(2);
            ...
            >
                ...
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">
                        Booking Reference: {reservation.booking_reference || `BK-${reservation.id}`}
                    </h3>
                    <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${getPaymentStatusBadge(reservation.payment_status || 'pending')}`}
                    >
                        Payment: {reservation.payment_status ? reservation.payment_status.replace(/_/g, ' ') : 'Pending'}
                    </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b pb-4 mb-4">
                    ...
                    <div>
                        ...
                        <p className="font-medium text-gray-800">
                            {format(checkOutDate, "MMM d, yyyy")} (Before 11:00 AM)
                        </p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-500">Total Nights</p>
                        <p className="font-medium text-gray-800">{totalNights} Nights</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b pb-4 mb-4">
                    <div>
                        ...
                        <p className="font-medium text-gray-800">{reservation.room?.hotel?.name || 'N/A'}</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-500">Room Type</p>
                        <p className="font-medium text-gray-800">{reservation.room?.room_type?.name || 'N/A'}</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-500">Room Number</p>
                        <p className="font-medium text-gray-800">{reservation.room?.room_number || 'Assigned at Check-in'}</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-500">Bed Configuration</p>
                        <p className="font-medium text-gray-800">{reservation.room?.bed_configuration || '1 King Bed'}</p> {/* Placeholder */}
                    </div>
                    <div>
                        <p className="text-sm text-gray-500">Guests</p>
                        <p className="font-medium text-gray-800">{reservation.num_adults} Adults, {reservation.num_children || 0} Children</p>
                    </div>
                </div>
                ...
                <div className="space-y-2 mb-4">
                    ...
                    <div className="flex justify-between text-gray-700">
                        <span>Base Price ({basePricePerNight} x {totalNights} nights)</span>
                        <span>${(basePricePerNight * totalNights).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-gray-700">
